...
#define SDIO_READ_TIMEOUT_MS    100
#define SDIO_WRITE_TIMEOUT_MS   250
#define SDIO_CLOCK_USB          48000000
#define SDIO_CLOCK_NO_USB       50000000

// Rx Error Mask:
// SDIO_STA_RXUNDERR | SDIO_STA_DCRCFAIL | SDIO_STA_DTIMEOUT | SDIO_STA_STBITERR
#define SDIO_RX_ERR_MASK        0x22A
// Tx Error Mask:
// SDIO_STA_TXUNDERR | SDIO_STA_DCRCFAIL |  SDIO_STA_DTIMEOUT | SDIO_STA_STBITERR
#define SDIO_TX_ERR_MASK        0x21A
#define SDIO_STATUS_ERR_MASK    0xFDF98008
#define SDIO_ICR_CLEAR_FLAGS       0x7FF

static struct {
    uint32_t read_timeout, write_timeout, rca;
    uint8_t flags, error;
} sdio_config;
...
sdio_set_rate(uint32_t rate)
{
    ...
    if (CONFIG_MACH_STM32F103)
        // SDIO on the STM32F103 uses the HCLK frequency
        sdioclk = CONFIG_CLOCK_FREQ;
    else if (CONFIG_USBSERIAL)
        sdioclk = SDIO_CLOCK_USB;
    else
        sdioclk = SDIO_CLOCK_NO_USB;
    uint32_t div = DIV_ROUND_UP(sdioclk, rate);
    div = (div > 256) ? 255 : ((div >= 2) ? (div - 2) : 0);
    SDIO->CLKCR = (1 << 8) | (div & 0xFF);
    uint32_t rate_actual = sdioclk / (div + 2);
    sdio_config.read_timeout = (rate_actual / 1000) * SDIO_READ_TIMEOUT_MS;
    sdio_config.write_timeout = (rate_actual / 1000) * SDIO_WRITE_TIMEOUT_MS;
}
...
sdio_init(void)
{
    ...
    // Init SDIO GPIOs
    gpio_peripheral(GPIO_SDIO_CMD, GPIO_FUNCTION(12), 0);
    gpio_peripheral(GPIO_SDIO_D0, GPIO_FUNCTION(12), 0);
    gpio_peripheral(GPIO_SDIO_D1, GPIO_FUNCTION(12), 0);
    gpio_peripheral(GPIO_SDIO_D2, GPIO_FUNCTION(12), 0);
    gpio_peripheral(GPIO_SDIO_D3, GPIO_FUNCTION(12), 0);
    gpio_peripheral(GPIO_SDIO_CLK, GPIO_FUNCTION(12), 0);

    // Initialize rate to 400K
    sdio_set_rate(400000);
    // Power on SDIO
    SDIO->POWER = 0x3;
}
...
sdio_send_command(uint8_t command, uint32_t arg)
{
    ...
    if (resp_len) {
        ...
        if (resp_type == 1 && (SDIO->RESP1 && SDIO_STATUS_ERR_MASK))
        {
            return SDIO_RESPONSE_STA_ERR;
        }
    }
...
wait_programming_done(void)
{
    ...
    while (tries) {
        ...
        if (ret != SDIO_RESPONSE_SUCCESS || (response & SDIO_STATUS_ERR_MASK))
            tries--;
...
sdio_write_block(uint32_t* write_buf, uint32_t address)
{
    ...
    SDIO->ICR = SDIO_ICR_CLEAR_FLAGS;
    SDIO->DTIMER = sdio_config.write_timeout;
    ...
    while (1) {
        status = SDIO->STA;
        if (status & SDIO_STA_TXFIFOHE &&
            (word_count < (SD_SECTOR_SIZE / 4)))
        {
            SDIO->FIFO = write_buf[word_count++];
            SDIO->FIFO = write_buf[word_count++];
            SDIO->FIFO = write_buf[word_count++];
            SDIO->FIFO = write_buf[word_count++];
            SDIO->FIFO = write_buf[word_count++];
         ...
         if (status & SDIO_TX_ERR_MASK) {
            ret = -4;
            sdio_config.error = SDIO_ERROR_WRITE_BLOCK;
            break;
        } else if(status & SDIO_STA_DBCKEND)
            break;
    }
    // Wait for card to finish transmitting/programming
    if (!wait_programming_done()) {
        ret = -5;
        sdio_config.error = SDIO_ERROR_WRITE_BLOCK;
    }
...
sdio_read_block(uint32_t* read_buf, uint32_t address)
{
    ...
    SDIO->DTIMER = sdio_config.read_timeout;
    SDIO->DLEN = SD_SECTOR_SIZE;
    ...
    while (1) {
        status = SDIO->STA;
        // Read out 8 words when FIFO is half full to maximize
        // performance and prevent buffer overrun
        if (status & SDIO_STA_RXFIFOHF) {
            read_buf[word_count++] = SDIO->FIFO;
            read_buf[word_count++] = SDIO->FIFO;
            read_buf[word_count++] = SDIO->FIFO;
            read_buf[word_count++] = SDIO->FIFO;
            read_buf[word_count++] = SDIO->FIFO;
    ...
    // Clear the buffer remaining buffer
    while (SDIO->STA & SDIO_STA_RXDAVL)
        read_buf[word_count++] = SDIO->FIFO;

    // Verify we received the full sector
   if (ret == SDIO_RESPONSE_SUCCESS && (word_count != (SD_SECTOR_SIZE / 4))) {
        ret = -5;
        sdio_config.error = SDIO_ERROR_READ_BLOCK;
    }
    // clear transfer state
...
sdcard_init(void)
{
    ...
    uint8_t arg = ((sd_ver == 1) ? 0 : (1 << 30)) | (3 << 20);
    uint32_t ocr_expect = (1 << 31) | (3 << 20);
    ...
    if (check_command(SDCMD_SET_CLR_CD_DETECT, 0, 1, 0, 0, 1))
    ...
    if (sdio_send_command(SDCMD_SET_BLOCKLEN, SD_SECTOR_SIZE) < 0) {
