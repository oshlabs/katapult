...
#include "board/flash.h" // application_read_flash
...
enum {SDS_NEED_UPLOAD = 1, SDS_NEED_VERIFY, SDS_DONE};
...
sdcard_open(void)
{
    ...
    res = f_open(&(ff_data.file_obj), "firmware.bin", 1);
    if (res != FR_OK)
        goto fail;
    ff_data.file_open = 1;
    ff_data.flash_state = SDS_NEED_UPLOAD;
    ff_data.block_address = CONFIG_APPLICATION_START;
...
static void
rename_firmware(const char* new_fname)
{
    if (ff_data.file_open) {
        f_close(&ff_data.file_obj);
        ff_data.file_open = 0;
    ...
    f_unlink(new_fname);
    f_rename("firmware.bin", new_fname);
}
...
{
    uint32_t sd_buf[CONFIG_BLOCK_SIZE / 4];
    uint32_t flash_buf[CONFIG_BLOCK_SIZE];
    memset(sd_buf, 0xFF, CONFIG_BLOCK_SIZE);
    UINT bcount;
    if (f_read(&(ff_data.file_obj), sd_buf, CONFIG_BLOCK_SIZE,
               &bcount) != FR_OK) {
